...
#import "DropDownTabelViewCell.h"
...
- (id)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id) delegate
{
    ...
    if (self) {
        ...
        for (int i = 0; i <sectionNum; i++) {
            ...
            sectionBtn.frame = CGRectMake(0, 0, sectionWidth, frame.size.height);
            ...
            [sectionBtn setTitleShadowColor:[UIColor colorWithWhite:0.0 alpha:0.2] forState:UIControlStateNormal];
...
//            UIImageView *sectionBtnIv = [[UIImageView alloc] initWithFrame:CGRectMake(sectionWidth*i +(sectionWidth - 16), (self.frame.size.height-12)/2, 12, 8)];
            UIImageView *sectionBtnIv = [[UIImageView alloc] initWithFrame:CGRectMake(90, 17, 12, 8)];
...
-(void)showChooseListViewInSection:(NSInteger)section choosedIndex:(NSInteger)index
{
    if (!self.mTableView) {
        ...
        self.mTableBaseView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height , self.frame.size.width, self.mTableBaseViewHeight)];
        self.mTableBaseView.backgroundColor = [UIColor colorWithWhite:0.0f alpha:0.3];
        
        UITapGestureRecognizer *bgTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(bgTappedAction:)];
        [self.mTableBaseView addGestureRecognizer:bgTap];
        
        self.mTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, self.mTableBaseViewHeight) style:UITableViewStylePlain];
    ...
    rect.size.height = 0;
    self.mTableView.frame = rect;
    clearTableViewOtherLine(self.mTableView);
...
void clearTableViewOtherLine(UITableView * tableview){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DropDownTabelViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
    if (!cell) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"DropDownTabelViewCell" owner:self options:nil] lastObject];
        cell.accessoryType = UITableViewCellAccessoryNone;
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    }
    cell.backgroupImageView.image = self.cellBackgroupImage;
    cell.showLabe.text = [self.dropDownDataSource titleInSection:currentExtendSection index:indexPath.row];
    cell.showLabe.textAlignment = NSTextAlignmentCenter;
    cell.showLabe.font = [UIFont systemFontOfSize:14];
    cell.line.backgroundColor = self.cellLineColor;
    return cell;
}
