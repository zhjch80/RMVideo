...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [Flurry logEvent:@"VIEW_MyChannel_Film" timed:YES];
}

- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    [Flurry endTimedEvent:@"VIEW_MyChannel_Film" withParameters:nil];
}
...
- (void)requestData{
    if(!isDownLoad){
        ...
        //排行类型（1：日榜 2：周榜 3：月榜）
        [manager getTagOfVideoListWithID:self.downLoadID andVideoType:@"1" WithPage:[NSString stringWithFormat:@"%d",pageNum]  count:@"12"];
        isDownLoad = YES;
    }
}
...
- (void)playBtnWithIndex:(NSInteger)index andLocation:(NSInteger)location{
    ...
    if([model.downLoadURL isEqualToString:@""]||model.downLoadURL== nil){
        ...
        [RMPlayer presentVideoPlayerWithPlayModel:playmodel withUIViewController:myChannelShouldDelegate withVideoType:1];
    }
}
